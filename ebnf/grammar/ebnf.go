...
type (
	...
	Grammar struct {
		...
		order       []string // production names in order of definition
	}
)
...
// Get returns the production with the given name, or nil if not found.
func (g Grammar) Get(name string) *Production {
	return g.productions[name]
}

// Set adds or updates a production in the grammar.
// If the production doesn't exist, it's added at the end of the order list.
func (g *Grammar) Set(name string, prod *Production) {
...
// Has returns true if the grammar contains a production with the given name.
...
// Order returns the production names in their definition order.
func (g Grammar) Order() []string {
	return g.order
}

// Range iterates over productions in definition order.
func (g Grammar) Range(fn func(name string, prod *Production) bool) {
	for _, name := range g.order {
		if !fn(name, g.productions[name]) {
			break
		}
	}
}
...
func (v *verifier) push(prod *Production) {
	...
	if !v.reached.Has(name) {
		v.worklist = append(v.worklist, prod)
		v.reached.Set(name, prod)
	}
}
...
func (v *verifier) verifyExpr(expr Expression, lexical bool) {
	...
	case *Name:
		...
		if prod := v.grammar.Get(x.String); prod != nil {
			v.push(prod)
		} else {
...
func (v *verifier) verify(grammar Grammar, start string) {
	...
	if v.reached.Len() < v.grammar.Len() {
		v.grammar.Range(func(name string, prod *Production) bool {
			if !v.reached.Has(name) {
